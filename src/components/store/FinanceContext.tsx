...
interface FinanceContextType {
  ...
  updateHouseholdMember: (
    memberId: string,
    updates: { role?: 'owner' | 'partner'; action?: 'remove' },
  ) => Promise<void>;
...
export const FinanceProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  // Fetch BDE Data on mount (or when data is stale)
  useEffect(() => {
    ...
    const isStale = data.marketData.lastUpdated
      ? new Date().getTime() - new Date(data.marketData.lastUpdated).getTime() >
        24 * 60 * 60 * 1000
      : true;
  ...
  }, [data.accounts, data.user, data.household, isInitialized]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  const saveToServer = async (currentData: FinanceData) => {
    ...
    const ownedAccounts = currentData.accounts.filter(
      (a) => a.ownerId === currentData.user?.id,
    );
    const ownedCosts = currentData.recurringCosts.filter(
      (c) => c.ownerId === currentData.user?.id,
    );

    // Goals are shared for now
    ...
    try {
      ...
      const saveResponse = await fetch(
        `https://${projectId}.supabase.co/functions/v1/make-server-d9780f4d/finance/save`,
        {
          method: 'POST',
      ...
      if (currentData.household && currentData.user.role === 'owner') {
        const householdResponse = await fetch(
          `https://${projectId}.supabase.co/functions/v1/make-server-d9780f4d/household/save`,
          {
            method: 'POST',
            headers: authHeaders(session.access_token),
            body: JSON.stringify({
        ...
        if (!householdResponse.ok) {
          const errorData = await householdResponse
            .json()
            .catch(() => ({ error: 'Unknown error' }));
          throw new Error(
            errorData.error || `Household save failed with status ${householdResponse.status}`,
          );
        }
      }
    } catch (e: any) {
  ...
  const loadFromServer = async (
    userId: string,
    options: LoadOptions = { skipHousehold: false, silent: true },
  ) => {
    const { skipHousehold = false, silent = true } = options;
    ...
    try {
      ...
      const response = await fetch(
        `https://${projectId}.supabase.co/functions/v1/make-server-d9780f4d/finance/load-household/${userId}`,
        {
          method: 'GET',
          headers: authHeaders(session.access_token),
        },
        ...
        if (found && serverData) {
          setData((prev) => {
            ...
            return {
              ...
              ...serverData,
              user: mergedUser,
              household: skipHousehold ? null : serverData.household,
              // Keep local market data
              marketData: prev.marketData,
            };
          });

          // Only show success toast explicitly when requested (manual refresh)
          if (!silent) {
            toast.success('Data refreshed');
          }
        } else {
          console.warn(`⚠️ No data found on server for user ${userId}`);
          // No toast here; silent even on manual refresh to avoid noise
        }
      } else {
      ...
      if (session?.user) {
        ...
        setData((prev) => {
          ...
          if (prev.user?.id === session.user.id) {
            console.log(
              'Auth state change detected but user ID unchanged - keeping existing data',
            );
        ...
        setData((prev) => {
          if (prev.user?.id !== session.user.id) {
            ...
            loadFromServer(session.user.id, { skipHousehold: true, silent: true });
          } else {
            setIsInitialized(true);
          }
          return prev;
  ...
  const createHousehold = async (name: string) => {
    ...
    try {
      ...
      const response = await fetch(
        `https://${projectId}.supabase.co/functions/v1/make-server-d9780f4d/household/create`,
        {
          method: 'POST',
          headers: authHeaders(session.access_token),
          body: JSON.stringify({
  ...
  const joinHousehold = async (code: string) => {
    ...
    try {
      ...
      const response = await fetch(
        `https://${projectId}.supabase.co/functions/v1/make-server-d9780f4d/household/join`,
        {
          method: 'POST',
          headers: authHeaders(session.access_token),
          body: JSON.stringify({
  ...
  const enterHousehold = async (household: Household) => {
    ...
    try {
      ...
      // Load full household data including accounts, costs, goals (silent)
      await loadFromServer(data.user.id, { skipHousehold: false, silent: true });
    } catch (e) {
  ...
  const checkServerHousehold = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `https://${projectId}.supabase.co/functions/v1/make-server-d9780f4d/household/user/${data.user.id}`,
        {
          method: 'GET',
          headers: authHeaders(session.access_token),
        },
  ...
  const updateHouseholdMember = async (
    ...
    try {
      ...
      const response = await fetch(
        `https://${projectId}.supabase.co/functions/v1/make-server-d9780f4d/household/member`,
        {
          method: 'POST',
          headers: authHeaders(session.access_token),
          body: JSON.stringify({
      ...
      // Refetch all data (silent) to ensure we don't lose merged member details
      await loadFromServer(data.user.id, { silent: true });
  ...
  const updateHouseholdSettings = async (settings: { name: string }) => {
    ...
    try {
      ...
      const response = await fetch(
        `https://${projectId}.supabase.co/functions/v1/make-server-d9780f4d/household/update`,
        {
          method: 'POST',
          headers: authHeaders(session.access_token),
          body: JSON.stringify({
  ...
  const addRecurringCost = (cost: Omit<RecurringCost, 'id'>) => {
    ...
    mutateAndSave((prev) => ({
      ...prev,
      recurringCosts: [...prev.recurringCosts, newCost],
    }));
  };
  ...
  const updateRecurringCost = (id: string, updates: Partial<RecurringCost>) => {
    mutateAndSave((prev) => ({
      ...prev,
      recurringCosts: prev.recurringCosts.map((c) =>
        c.id === id ? { ...c, ...updates } : c,
      ),
    }));
  };
  ...
  const addIncomeSource = (source: Omit<IncomeSource, 'id'>) => {
    mutateAndSave((prev) => {
      ...
      updatedUser.netIncome = updatedUser.incomeSources.reduce(
        (sum, s) => sum + s.amount,
        0,
      );
  ...
  const updateIncomeSource = (id: string, updates: Partial<IncomeSource>) => {
    mutateAndSave((prev) => {
      ...
      const updatedSources = prev.user.incomeSources.map((s) =>
        s.id === id ? { ...s, ...updates } : s,
      );
      const updatedUser = {
        ...prev.user,
  ...
  const getPersonalTotalIncome = () => {
    ...
    if (data.user.incomeSources && data.user.incomeSources.length > 0) {
      return data.user.incomeSources.reduce(
  ...
  const getHouseholdIncome = () => {
    ...
    return data.household.members.reduce((sum, member) => {
      ...
      if (userToUse.incomeSources && userToUse.incomeSources.length > 0) {
        return (
          sum +
          userToUse.incomeSources.reduce(
            (s, source) => s + Number(source.amount || 0),
            0,
          )
        );
      }
  ...
  const refreshData = async () => {
    if (!data.user) return;
    // Manual refresh: explicit success toast allowed
    await loadFromServer(data.user.id, { silent: false, skipHousehold: false });
  };
